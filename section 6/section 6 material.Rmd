...
```{r}
# Figure out what you want your function to do
na_stats <- function(x) {
  # return the proportion of NAs, as well as the count
  prop <- mean(is.na(x))
  # print(is.na(x))
  count <- sum(is.na(x))
...
# na_stats(x = pdxTrees$Tree_Height)
na_stats(x = c(NA, 3, 2, 1))
```
...
thing <- function(na.rm = FALSE, x = c(7, 1), y = c(3, 4)) {
  ...
  return(pt(q = abs(important_bit), df = df, lower.tail = FALSE)*2)
}
## Test it!
...
```{r}
t_test <- function(x, y, na.rm = TRUE) {
  ...
  n_y <- length(y)
  df <- min(n_x, n_y) - 1
  test_stat <- (mean_x - mean_y - 0)/sqrt(var_x/n_x + var_y/n_y)
  return(pt(q = abs(test_stat), df = df, lower.tail = FALSE)*2)
}

t_test(na.rm = TRUE, x = x, y = y)
t_test()
```
